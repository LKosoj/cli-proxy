...
- затем `git commit -m "<краткое>" -m "<детальное>"`.

Бот попросит прислать сообщение коммита отдельным сообщением.  
Если изменений нет — коммит не выполняется.  
Для отмены отправьте `-`.

Детальное описание формируется автоматически (по `git diff --stat` и `git status --porcelain`),
а если есть доступ к модели — может быть предложено более развернутое описание (3–6 пунктов).
