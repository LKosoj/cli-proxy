...
def _wrap_html(body: str, theme_colors: Optional[Dict[str, str]] = None) -> str:
    ...
    return (
        ...
        f"line-height:1.5;color:{colors['text_primary']};background:{colors['bg_primary']};"
        "margin:0;padding:16px;overflow-wrap:break-word;}"
        "main{max-width:100%;}"
        ...
        f"pre{{background:{colors['bg_tertiary']};padding:12px;border-radius:6px;"
        "overflow:auto;max-width:100%;white-space:pre;-webkit-overflow-scrolling:touch;}"
        f"code{{background:{colors['bg_tertiary']};padding:2px 4px;border-radius:4px;}}"
        "pre code{background:transparent;padding:0;border-radius:0;white-space:inherit;"
        "overflow-wrap:normal;word-break:normal;}"
        "table{border-collapse:collapse;margin:12px 0;display:block;max-width:100%;"
        ...
        "@media (max-width:600px){body{padding:12px;font-size:15px;}th,td{padding:6px 8px;}"
