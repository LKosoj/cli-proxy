...
class ManagerOrchestrator:
    ...
    def _snapshot_workdir(
        ...
        *,
        ...
        return self._review_service.snapshot_workdir(
            workdir,
            max_files=max_files,
    ...
    async def _auto_commit_baseline_before_first_step(
        ...
        code, commit_out = await self._run_git(workdir, ["commit", "-m", summary_line])
        if code != 0:
        ...
        if chat_id is not None:
            await self._send_adapter_message(bot, context, chat_id=chat_id, text=f"🧷 Бейзлайн-коммит: {summary_line}")
        return True
    ...
    async def _reconcile_plan_after_commit(
        ...
        remaining_tasks_info = self._review_service.remaining_tasks_info(plan)
        if not remaining_tasks_info:
            return

        # Get last commit info (message + hash + file stat) to ground reconciliation.
        # Keep it consistent with the developer prompt's "Последние изменения".
        code, log_out = await self._run_git(workdir, ["log", "-1", "--stat", "--format=%s (%h)"])
        if code != 0:
            log_out = ""
        ...
        raw = await chat_completion(
            self._config,
            self._manager_prompt(workdir, "plan_reconcile_system"),
            user_msg[:12000],
            response_format={"type": "json_object"},
        )
        ...
        apply_result = self._review_service.apply_reconcile_payload(
            plan,
            payload,
            now_iso=_now_iso,
        )
        ...
        if analysis_changed:
            ...
            # Keep log small but useful: preview the key change that prevents stale context hallucinations.
        ...
        if changes_made:
            ...
            if chat_id is not None:
                ...
                await self._send_adapter_message(bot, context, chat_id=chat_id, text="\n".join(lines))
    ...
    async def _reconcile_plan_after_change_audit(
        ...
        """
        If git isn't used, we still want to detect "CLI did more than asked" and adjust remaining tasks.
        We ground the reconciliation on a filesystem change audit captured during the dev step.
        """
        chat_id = dest.get("chat_id")
        workdir = session.workdir
        archive_enabled = bool(getattr(self._config.defaults, "manager_response_archive", False))
