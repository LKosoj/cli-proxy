...
- Проверяет, что агент активен (в текущей сессии `active_mode == "agent"`).
...
Этот вызов заменяет устаревший `_base_cancel_commands()`. Telegram-команды больше не нужны — действия плагина доступны через двухуровневое inline-меню (`get_menu_label` + `get_menu_actions`).
...
**Не нужно** вызывать `_ensure_agent_active()` в обработчиках вручную — это сделает базовый класс.
...
| `_ensure_agent_active(context) → bool` | Проверка активности агента (вызывается автоматически) |
...
1. **Изменении inventory сессий** (`SessionManager.create()/close()/rename()` и scope/thread reconcile) — callback `on_session_change` вызывает `cancel_all_inputs()` для всех чатов.
2. **Создании новой сессии** (`SessionManager.create()`) — аналогично.
3. **Закрытии сессии** (`SessionManager.close()`) — аналогично.
4. **Выключении агента** (через `mode_action:agent:disable`) — явный вызов `_cancel_plugin_dialogs(chat_id)`.
