...
# Step handler can be either a plain callable (message-only)
...
class DialogMixin:
    ...
    * ``get_inline_handlers()`` returning a single CallbackQueryHandler config
    ...
    def _ensure_agent_active(self, context: Any, update: Any = None) -> bool:
        ...
        try:
            ...
            if update is not None:
                ...
                raw_chat_id = getattr(effective_chat, "id", None)
                ...
                if raw_chat_id is None:
                    ...
                    raw_chat_id = getattr(effective_message, "chat_id", None)
            ...
            session = (
                resolver(reply_chat_id=chat_id, message_thread_id=thread_id, owner_chat_id=chat_id)
                if callable(resolver)
                else None
            )
            return bool(bot_app and bot_app._mode_allows_plugin_ui(session))
        except Exception:
            return False
    ...
    async def handle_message(self, update: Any, context: Any) -> bool:
        ...
        # Agent-active guard: auto-cancel dialog if agent was turned off.
        if not self._ensure_agent_active(context, update):
            self.end_dialog(chat_id)
            if msg:
    ...
    async def _dispatch_callback(self, update: Any, context: Any) -> None:
        """Single entry point registered by ``get_inline_handlers``.
        ...
        # 0. Agent-active guard: silently reject if agent is off.
        if not self._ensure_agent_active(context, update):
            try:
    ...
    def get_inline_handlers(self) -> List[Dict[str, Any]]:  # type: ignore[override]
        pid = re.escape(self._plugin_id_safe())
        pattern = f"^(dlg_cancel|dlg|cb):{pid}(:|$)"
        return [{"pattern": pattern, "handler": self._dispatch_callback}]
