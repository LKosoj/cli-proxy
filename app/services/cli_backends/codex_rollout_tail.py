...
От `CliTranscriptReader` отличается тем, что перечитывает файл целиком, а не
идёт по курсору: у headless-запуска нет живого pane, за которым нужно поспевать.
Завершение хода, как и у читателя транскриптов, определяется событием
`task_complete`.
"""
