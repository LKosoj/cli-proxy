...
- Отчёты текущей сессии доступны в Telegram через `/reports`; `/reports snapshot` и кнопка "Отчёт сессии" в меню `/sessions` создают HTML-снимок сессии из run artifacts, истории отчётов и доступного transcript CLI. Финальный отчёт Manager автоматически сохраняется в общую историю отчётов.
...
4. **Работа с результатом**
   ...
   - Отчёты сессии смотрите через `/reports`; `/reports latest` отправляет последний отчёт, `/reports snapshot` создаёт HTML-снимок сессии, `/reports generate` создаёт отчёт из текущего Manager-плана, `/reports <filename.md>` отправляет конкретный файл.
