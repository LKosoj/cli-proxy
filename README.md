...
2) **Отправка задачи**
   ...
   - Вложения `.txt`, `.md`, `.rst`, `.log` (до 300 КБ) отправляются сразу и объединяются с описанием.
...
## Вложения сообщений
- Текстовые вложения: `.txt`, `.md`, `.rst`, `.log` (до 300 КБ). Бот объединяет подпись и содержимое файла и отправляет в CLI.
