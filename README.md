...
- Короткий вывод до 3900 символов отправляется обычным Telegram-сообщением; длинный вывод или `force_html=True` отправляется HTML-файлом (Markdown-разметка, ANSI-цвета, mermaid-диаграммы в SVG; при ошибке рендера остаются как код-блоки).
...
- Отчёты текущей сессии доступны в Telegram через `/reports`; `/reports snapshot` и кнопка "Отчёт сессии" в меню `/sessions` создают человеко-читаемый HTML-отчёт по сессии из run artifacts, истории отчётов и доступного transcript CLI. Финальный отчёт Manager автоматически сохраняется в общую историю отчётов.
...
4. **Работа с результатом**
   - Короткий результат до 3900 символов приходит обычным Telegram-сообщением.
   ...
   - Отчёты сессии смотрите через `/reports`; `/reports latest` отправляет последний отчёт, `/reports snapshot` создаёт человеко-читаемый HTML-отчёт по сессии, `/reports generate` создаёт отчёт из текущего Manager-плана, `/reports <filename.md>` отправляет конкретный файл.
