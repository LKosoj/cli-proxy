...
class MessageProcessor:
    ...
    async def on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if chat_id in self.bot_app.pending_git_clone:
            ...
            url = text.strip()
            if not is_within_root(base, self.bot_app.dirs_root.get(chat_id, self.bot_app.config.defaults.workdir)):
                await self.bot_app._send_message(context, chat_id=chat_id, text="Нельзя выйти за пределы корневого каталога.")
                return
            if not os.path.isdir(base):
                await self.bot_app._send_message(context, chat_id=chat_id, text="Каталог не существует.")
            ...
            return
        if await self._plugin_awaiting_input(chat_id):
    ...
    def _plugin_awaiting_input(self, chat_id: int) -> bool:
        ...
        registry = getattr(self.bot_app, "_tool_registry", None)
        if registry is None:
            return False
        try:
