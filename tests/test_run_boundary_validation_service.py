...
from modes.codebase_mapper_constants import CODEBASE_MAPPER_RESULT_STATUS
...
def test_run_boundary_validation_codebase_mapper_operation_passes_for_consistent_map_dir(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_ok",
    ...
    (map_dir / "state.json").write_text(json.dumps(_mapper_state_payload(nodes_count=3), ensure_ascii=False), encoding="utf-8")
    (map_dir / "graph.json").write_text(
        '{"nodes":[{"id":"n1"},{"id":"n2"},{"id":"n3"}],"edges":[],"tree":[]}',
        encoding="utf-8",
    )
    ...
    report = service.validate(run, mode_id="codebase_mapper", phase="operation")
...
def test_run_boundary_validation_codebase_mapper_operation_passes_for_live_runtime_statuses(
    tmp_path,
    runtime_status: str,
) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id=f"codebase_mapper_operation_status_{runtime_status}",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    (map_dir / "state.json").write_text(
        json.dumps(_mapper_state_payload(state="ready", nodes_count=2), ensure_ascii=False),
        encoding="utf-8",
    )
    (map_dir / "graph.json").write_text(
...
def test_run_boundary_validation_codebase_mapper_operation_fails_when_repair_leaves_validate_queue(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_bad",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    (map_dir / "graph.json").write_text('{"nodes":[{"id":"n1"}],"edges":[],"tree":[]}', encoding="utf-8")
    (map_dir / "INDEX.md").write_text("# Index\n", encoding="utf-8")
    store.save_state(
        run,
        {
    ...
    assert report.status == "error"
    assert any(issue.code == "validation_failed" for issue in report.issues)
    assert any(issue.code == "manual_review_pending" for issue in report.issues)


def test_run_boundary_validation_codebase_mapper_operation_fails_when_graph_json_missing(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_missing_graph",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    assert report.status == "error"
    assert any(issue.code == "graph_corrupted" for issue in report.issues)


def test_run_boundary_validation_codebase_mapper_operation_fails_when_graph_json_is_malformed(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_bad_graph_json",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    (map_dir / "meta.json").write_text('{"version":1}', encoding="utf-8")
    (map_dir / "state.json").write_text(json.dumps(_mapper_state_payload(nodes_count=3), ensure_ascii=False), encoding="utf-8")
    (map_dir / "graph.json").write_text('{"nodes": ', encoding="utf-8")
    (map_dir / "INDEX.md").write_text("# Index\n", encoding="utf-8")
    store.save_state(
        run,
        {
...
def test_run_boundary_validation_codebase_mapper_operation_fails_when_graph_json_is_schema_weak(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_weak_graph_json",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    map_dir.mkdir(parents=True, exist_ok=True)
    (map_dir / "meta.json").write_text('{"version":1}', encoding="utf-8")
    (map_dir / "state.json").write_text(json.dumps(_mapper_state_payload(nodes_count=0), ensure_ascii=False), encoding="utf-8")
    (map_dir / "graph.json").write_text('{"tree":[]}', encoding="utf-8")
    (map_dir / "INDEX.md").write_text("# Index\n", encoding="utf-8")
    store.save_state(
        run,
        {
...
def test_run_boundary_validation_codebase_mapper_operation_fails_when_graph_json_disagrees_with_state(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_graph_state_mismatch",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    (map_dir / "meta.json").write_text('{"version":1}', encoding="utf-8")
    (map_dir / "state.json").write_text(
        json.dumps(_mapper_state_payload(nodes_count=2, tree=["root", "nodes"]), ensure_ascii=False),
        encoding="utf-8",
    )
    (map_dir / "graph.json").write_text(
        '{"nodes":[{"id":"n1"}],"edges":[],"tree":["root"]}',
        encoding="utf-8",
    )
    (map_dir / "INDEX.md").write_text("# Index\n", encoding="utf-8")
    store.save_state(
...
def test_run_boundary_validation_codebase_mapper_operation_accepts_sparse_legacy_state_json(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_legacy_sparse_state_json",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    (map_dir / "meta.json").write_text('{"version":1}', encoding="utf-8")
    (map_dir / "state.json").write_text(
        json.dumps({"state": "ready", "operation": "verify", "nodes_count": 1, "tree": []}, ensure_ascii=False),
        encoding="utf-8",
    )
    (map_dir / "graph.json").write_text(
        '{"nodes":[{"id":"n1"}],"edges":[],"tree":[]}',
        encoding="utf-8",
    )
    (map_dir / "INDEX.md").write_text("# Index\n", encoding="utf-8")
    store.save_state(
        run,
        {
            ...
            "mode_context": {
                "operation": "verify",
                "map_dir": str(map_dir),
                "status": CODEBASE_MAPPER_RESULT_STATUS["GRAPH_VERIFIED"],
                "validate_queue": [],
                "needs_review": [],
            },
        },
...
def test_run_boundary_validation_codebase_mapper_operation_fails_when_legacy_optional_field_type_is_invalid(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_legacy_optional_field_invalid",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
...
def test_run_boundary_validation_codebase_mapper_operation_fails_when_state_json_misses_required_fields(tmp_path) -> None:
    _cfg, store, service, _session, run = _prepare_run(
        tmp_path,
        case_id="codebase_mapper_operation_bad_state_json",
        mode_id="codebase_mapper",
        phase="operation",
    )
    map_dir = Path(run.run_dir) / "artifacts" / "mapper-map"
    ...
    (map_dir / "meta.json").write_text('{"version":1}', encoding="utf-8")
    (map_dir / "state.json").write_text(
        json.dumps({"state": "validated", "needs_review": []}, ensure_ascii=False),
        encoding="utf-8",
    )
    (map_dir / "graph.json").write_text(
        '{"nodes":[{"id":"n1"}],"edges":[],"tree":[]}',
