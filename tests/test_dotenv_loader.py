...
from app.services.dotenv_loader import load_dotenv, parse_dotenv
    ...
    (tmp_path / "config.yaml").write_text(
        "\n".join(
            [
                ...
                f"  state_path: '{(tmp_path / 'state.json').as_posix()}'",
                f"  toolhelp_path: '{(tmp_path / 'toolhelp.txt').as_posix()}'",
                f"  log_path: '{(tmp_path / 'bot.log').as_posix()}'",
                "mcp: { enabled: false }",
                "mcp_clients: []",
                "presets: []",
