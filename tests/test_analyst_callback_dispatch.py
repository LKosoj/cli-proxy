...
def _build_mode(bot_app) -> AnalystMode:
    mode = AnalystMode()
    mode.initialize(
...
def test_analyst_handle_callback_uses_dispatcher_and_cb_handler(monkeypatch) -> None:
    async def _run() -> None:
        class _FakeBotApp:
            def __init__(self):
...
def test_analyst_handle_callback_on_alias_dispatches_enable(monkeypatch) -> None:
    async def _run() -> None:
        class _FakeBotApp:
            def __init__(self):
                self.config = types.SimpleNamespace(defaults=types.SimpleNamespace())
        ...
        seen = {"enable_called": 0}

        async def _fake_cb_enable(**_kwargs):
            seen["enable_called"] += 1
            return ToolResult.ok()

        monkeypatch.setattr(mode, "_cb_enable", _fake_cb_enable)
        result = await mode.handle_callback(
            CallbackModel(action="on", chat_id=123, user_id=99, payload={}),
            {
                "bot_app": bot_app,
                "session": session,
                "chat_id": 123,
        ...
        assert result.success is True
        assert seen["enable_called"] == 1

    asyncio.run(_run())


def test_analyst_handle_callback_unknown_action_returns_fail() -> None:
    async def _run() -> None:
        class _FakeBotApp:
            def __init__(self):
                self.config = types.SimpleNamespace(defaults=types.SimpleNamespace())
