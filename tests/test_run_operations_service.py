...
@pytest.mark.asyncio
async def test_run_operations_resume_blocks_admin_boundary_failure_as_manual_review(tmp_path) -> None:
    cfg = _build_config(tmp_path)
    session = _build_session(tmp_path)
    artifact_store = RunArtifactStore(cfg)
    boundary = RunBoundaryValidationService(enabled=True)
    ...
    run = artifact_store.start_run(
        session=session,
        mode_id="admin",
        run_id="run_20260313T201800Z_adminrestart",
        phase="analyze",
        source_prompt_hash="sha256:admin-analyze",
    )
    artifact_store.save_state(
        run,
        {
            ...
            "status": "running",
            "mode_context": {
                "operation_payload": {"kind": "watch_loop"},
                "snapshot_id": "snap-1",
                "snapshot_ids": ["snap-1"],
                "snapshot_fidelity": {
                    "snapshot_id": "snap-2",
                ...
                "last_monitor_snapshot": {"server_count": 1},
                "last_analyzer_decision": {"status": "alert"},
            },
        },
    )
