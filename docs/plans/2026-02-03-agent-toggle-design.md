...
- В `/status` отображается `Agent: ON|OFF` для активной сессии.
- В информации о выбранной сессии также выводится `Agent: ON|OFF`.
- Дефолт для новой сессии: **OFF**.

## Архитектура и хранение состояния
- Состояние хранится в объекте сессии: `agent_enabled: bool`.
- Состояние сохраняется **в файле сессий** как новый параметр.
- При рестарте процесса состояние поднимается из файла сессий для активной сессии.
- При выборе другой сессии как активной состояние **считывается из параметра выбранной сессии**.
- Дефолт: OFF, если параметра нет (для обратной совместимости).
